...
export function waiveSubscription(pharmacyId) {
  const subs = store.get('subscriptions');
...
export function unwaiveSubscription(pharmacyId) {
  const subs = store.get('subscriptions');
  const idx = subs.pharmacySubscriptions.findIndex(s => s.pharmacyId === pharmacyId);
  if (idx >= 0) {
...
export function changePharmPlan(pharmacyId) {
  const subs = store.get('subscriptions');
...
/** Apply plan change */
export function applyPlanChange(pharmacyId) {
...
export function processPayment(planId) {

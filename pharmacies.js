...
export function renderPharmacies() {
...
export function viewPharmacy(pharmId) {
...
/** Add pharmacy modal */
export function showAddPharmacyModal(pharmId = null) {
...
/** Save pharmacy */
export function savePharmacy(e, pharmId) {
  e.preventDefault();
  const pharmacies = store.get('pharmacies') || [];
  const data = {
...
export function togglePharmacyStatus(pharmId) {
  const pharmacies = store.get('pharmacies') || [];
  const idx = pharmacies.findIndex(p => p.id === pharmId);
  if (idx >= 0) {
...
export function editPharmacy(pharmId) {

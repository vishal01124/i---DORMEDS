...
class Store {
  ...
  _init() {
    const saved = localStorage.getItem('dormeds_state');
    if (saved) {
      try {
        this._state = JSON.parse(saved);
        // Merge any new default keys that might not exist in saved state
  ...
  _persist() {
    try {
      localStorage.setItem('dormeds_state', JSON.stringify(this._state));
    } catch (e) {
